...
def turn(local_pokemon, remote_pokemon):
    ...
    for move in local_pokemon.moves:
        print(move.name + " | " + str(round(move.power, 2)) + " | " +
              str(round(move.accuracy, 2)) + " | " + str(move.power_point))
...
if __name__ == "__main__":
    fire_moves = [Move("Tackle", 12.0, 100.0, 10), Move(
        "QuickAttack", 14.0, 100.0, 7), Move("Ember", 15.0, 100.0, 7)]
    eletric_moves = [Move("Thunderbolt", 15.0, 100.0, 7), Move(
        "QuickAttack", 14.0, 100.0, 7), Move("ThunderShock", 20.0, 100.0, 3)]
    grass_moves = [Move("Absorb", 8.0, 100.0, 10), Move(
        "LeafBlade", 20.0, 80.0, 5), Move("Tackle", 12.0, 100.0, 10)]
    water_moves = [Move("AquaTail", 12.0, 100.0, 10), Move(
        "Bubble", 14.0, 100.0, 7), Move("QuickAttack", 14.0, 100.0, 7)]
